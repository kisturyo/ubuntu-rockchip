...
tar -xpJf "ubuntu-${RELASE_VERSION}-${SUITES}-${FLAVOR}-arm64.rootfs.tar.xz" -C ${chroot_dir}
...
cd ${chroot_dir} && tar -cpf "../ubuntu-${RELASE_VERSION}-${SUITES}-${FLAVOR}-arm64-${BOARD}.rootfs.tar" . && cd .. && rm -rf ${chroot_dir}
../scripts/build-image.sh "ubuntu-${RELASE_VERSION}-${SUITES}-${FLAVOR}-arm64-${BOARD}.rootfs.tar"
rm -f "ubuntu-${RELASE_VERSION}-${SUITES}-${FLAVOR}-arm64-${BOARD}.rootfs.tar"
