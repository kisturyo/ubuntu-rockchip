...
if [[ -f ubuntu-${RELASE_VERSION}-lts-${FLAVOR}-arm64.rootfs.tar.xz ]]; then
    exit 0
fi
...
(cd chroot/ &&  tar -p -c --sort=name --xattrs ./*) | xz -3 -T0 > "ubuntu-${RELASE_VERSION}-lts-${FLAVOR}-arm64.rootfs.tar.xz"
mv "ubuntu-${RELASE_VERSION}-lts-${FLAVOR}-arm64.rootfs.tar.xz" ../
